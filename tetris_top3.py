...
class Tetris:
    ...
    def render(self):
        ...
        complete_board = self._get_complete_board()
        img = [Tetris.COLORS[p] for row in self._get_complete_board() for p in row]
        img = np.array(img).reshape(3, Tetris.BOARD_WIDTH, 3).astype(np.uint8)
        img = img[..., ::-1] # Convert RRG to BGR (used by cv2)
        img = Image.fromarray(img, 'RGB')
        img = img.resize((Tetris.BOARD_WIDTH * 25, 3 * 25), Image.NEAREST)
        img = np.array(img)
