...
class Tetris:
    ...
    def reset(self):
        ...
        self._new_round()
        self.score = 0
        self._add_obstacles()  # Add obstacles during reset
        return self._get_board_props(self.board)

    def _add_obstacles(self):
        '''Randomly adds obstacles to the board'''
        num_obstacles = random.randint(1,3)  # Random number of obstacles
        for _ in range(num_obstacles):
            x = random.randint(0, Tetris.BOARD_WIDTH - 1)
            y = random.randint(0, Tetris.BOARD_HEIGHT - 1)
            # Ensure the obstacle does not overlap with existing blocks
            if self.board[y][x] == Tetris.MAP_EMPTY:
                self.board[y][x] = Tetris.MAP_BLOCK  # Place an obstacle
    ...
    def _check_collision(self, piece, pos):
        ...
        for x, y in piece:
            ...
            if x < 0 or x >= Tetris.BOARD_WIDTH \
                    ...
                    or self.board[y][x] in (Tetris.MAP_BLOCK, Tetris.MAP_PLAYER):  # Check for obstacles
                return True
        return False
